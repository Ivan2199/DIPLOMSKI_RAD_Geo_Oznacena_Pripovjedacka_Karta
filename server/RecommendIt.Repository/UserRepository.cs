...
namespace GeoTagMap.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<PagingInfo<IUserModel>> GetAllUsersAsync(Paging paging, Sorting sort, UserFiltering filtering)
        {
            ...
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    query.Append("WHERE u.\"IsActive\" = true");
        ...
        public async Task<IUserModel> GetUserByIdAsync(Guid id)
        {
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    query.Append("AND u.\"IsActive\" = true");
        ...
        public async Task UpdateUserAsync(Guid userId, IUserModel user)
        {
            using (var con = new NpgsqlConnection(_connectionString))
            {
                await con.OpenAsync();
            ...
            await using (var connection = new NpgsqlConnection(_connectionString))
            {
                ...
                await using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        queryUser.Append("UPDATE \"User\" SET \"IsActive\" = false WHERE \"User\".\"Id\" = @id;");
                        cmd.CommandText = queryUser.ToString();
                        await cmd.ExecuteNonQueryAsync();

                        StringBuilder queryComment = new();
                        queryComment.Append("UPDATE \"Comment\" SET \"IsActive\" = false WHERE \"Comment\".\"CreatedBy\" = @id;");
                        cmd.CommandText = queryComment.ToString();
                        await cmd.ExecuteNonQueryAsync();

                        StringBuilder queryStory = new();
                        queryStory.Append("UPDATE \"Story\" SET \"IsActive\" = false WHERE \"Story\".\"CreatedBy\" = @id;");
                        cmd.CommandText = queryStory.ToString();
                        await cmd.ExecuteNonQueryAsync();

                        StringBuilder queryLocation = new();
                        queryLocation.Append("UPDATE \"Location\" SET \"IsActive\" = false WHERE \"Location\".\"CreatedBy\" = @id;");
                        cmd.CommandText = queryLocation.ToString();
                        await cmd.ExecuteNonQueryAsync();

                        StringBuilder queryGeoLocation = new();
                        queryGeoLocation.Append("UPDATE \"GeoLocation\" SET \"IsActive\" = false WHERE \"GeoLocation\".\"CreatedBy\" = @id");
                        cmd.CommandText = queryGeoLocation.ToString();
                        await cmd.ExecuteNonQueryAsync();

                        StringBuilder queryPhoto = new();
                        queryPhoto.Append("UPDATE \"Photo\" SET \"IsActive\" = false WHERE \"Photo\".\"CreatedBy\" = @id");
                        cmd.CommandText = queryPhoto.ToString();
                        await cmd.ExecuteNonQueryAsync();

                        StringBuilder queryLike = new();
                        queryLike.Append("UPDATE \"Like\" SET \"IsActive\" = false WHERE \"Like\".\"CreatedBy\" = @id");
                        cmd.CommandText = queryLike.ToString();
                        await cmd.ExecuteNonQueryAsync();
