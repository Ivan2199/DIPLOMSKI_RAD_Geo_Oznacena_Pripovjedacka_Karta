...
namespace GeoTagMap.WebApi.Controllers
{
    ...
    public class PhotoController : ApiController
    {
        ...
        [HttpGet]
        [Authorize(Roles = "Admin")]
        [Route("touristsite/{id}")]
        public async Task<HttpResponseMessage> GetPhotosByTouristSiteIdAsync(Guid id)
        {
            List<PhotoView> photoViews = new List<PhotoView>();
            try
            {
                var photos = await _photoService.GetPhotosByTouristSiteIdAsync(id);
                if (photos.Count() == 0)
                {
                    return Request.CreateResponse(HttpStatusCode.NoContent, "List is empty");
                }
        ...
        [Authorize(Roles = "Admin")]
        [Route("story/{id}")]
        public async Task<HttpResponseMessage> GetPhotosByStoryIdAsync(Guid id)
        {
            List<PhotoView> photoViews = new List<PhotoView>();
            try
            {
                var photos = await _photoService.GetPhotosByStoryIdAsync(id);
                if (photos.Count() == 0)
                {
                    return Request.CreateResponse(HttpStatusCode.NoContent, "List is empty");
                }
        ...
        public async Task<HttpResponseMessage> GetAsync(Guid id)
        {
            try
            {
                ...
                if (photo is null)
                {
                    return Request.CreateResponse(HttpStatusCode.NoContent, "No photo with that Id");
                }
        ...
        [HttpDelete]
        [Authorize(Roles = "Admin,")]
        public async Task<HttpResponseMessage> DeletePhotoAsync(Guid id)
        {
            try
            {
                if (await _photoService.GetPhotoAsync(id) == null)
                {
                    return Request.CreateResponse(HttpStatusCode.NotFound, "No photo with that id was found");
                }

                await _photoService.DeletePhotoAsync(id);

                return Request.CreateResponse(HttpStatusCode.OK, "Photo has been deleted successfully");
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
