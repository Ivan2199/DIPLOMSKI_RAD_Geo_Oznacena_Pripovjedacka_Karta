...
namespace GeoTagMap.WebApi.Controllers
{
    ...
    public class EventController : ApiController
    {
            ...
            )
        {
            ...
            EventFiltering filtering = new EventFiltering(name, eventStatus, startDate, type, isAccessibleForFree, searchKeyword, country, city);
        ...
        [Authorize(Roles = "Admin")]
        [Route("mostliked")]
        public async Task<HttpResponseMessage> GetMostLikedEventsAsync()
        {
            List<EventView> eventViews = new List<EventView>();
            try
            {
                var events = await _eventService.GetMostLikedEventsAsync();
                if (events.Count() == 0)
                {
                    return Request.CreateResponse(HttpStatusCode.NoContent, "List is empty");
                }
                ...
                return Request.CreateResponse(HttpStatusCode.OK, eventViews);
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
        ...
        private IEventModel MapEvent(EventRest eventRest)
        {
            return new EventModel
            {
                ...
                NumberOfLikes = eventRest.NumberOfLikes,
                IsActive = true,
