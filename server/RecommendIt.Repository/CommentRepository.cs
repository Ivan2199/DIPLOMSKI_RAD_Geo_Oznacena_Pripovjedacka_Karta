    ...
    public class CommentRepository : ICommentRepository
    {
        ...
        public async Task<PagingInfo<ICommentModel>> GetAllCommentsAsync(Paging paging, Sorting sort, CommentFiltering filtering)
        {
            ...
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var commentIdCmd = con.CreateCommand())
                {
                    StringBuilder commentIdQuery = new StringBuilder();
                    commentIdQuery.Append("SELECT c.\"Id\" FROM \"Comment\" c ");
                    commentIdQuery.Append("LEFT JOIN \"User\" u ON c.\"UserId\" = u.\"Id\" ");
                    commentIdQuery.Append("WHERE c.\"IsActive\" = true");

                    if (!string.IsNullOrEmpty(filtering.Username))
                    {
                        commentIdQuery.Append(" AND LOWER(u.\"Username\") = LOWER(@Username)");
                        commentIdCmd.Parameters.AddWithValue("@Username", filtering.Username.ToLower());
                    }
                    if (filtering.DateCreated != null)
                    {
                        commentIdQuery.Append(" AND c.\"DateCreated\" >= @DateCreated");
                        commentIdCmd.Parameters.AddWithValue("@DateCreated", filtering.DateCreated.Value);
                    }
                    if (filtering.IsReported.HasValue)
                    {
                        commentIdQuery.Append(" AND c.\"IsReported\" = @IsReported");
                        commentIdCmd.Parameters.AddWithValue("@IsReported", filtering.IsReported.Value);
                    }

                    if (!string.IsNullOrEmpty(sort.OrderBy))
                    {
                        commentIdQuery.Append($" ORDER BY c.\"{sort.OrderBy}\" {(sort.SortOrder.ToUpper() == "DESC" ? "DESC" : "ASC")}, c.\"Id\"");
                    }
                    else
                    {
                        commentIdQuery.Append(" ORDER BY c.\"Id\"");
                    }

                    commentIdQuery.Append(" LIMIT @Limit OFFSET @Offset");
                    commentIdCmd.Parameters.AddWithValue("@Limit", paging.RRP);
                    commentIdCmd.Parameters.AddWithValue("@Offset", (paging.PageNumber - 1) * paging.RRP);

                    commentIdCmd.CommandText = commentIdQuery.ToString();

                    using (var reader = await commentIdCmd.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            commentIds.Add(reader.GetGuid(reader.GetOrdinal("Id")));
                        }
                    }
                }
                ...
                {
                    return new PagingInfo<ICommentModel>
                    {
                        List = new List<ICommentModel>(),
                        RRP = paging.RRP,
                ...
                using (var countCmd = con.CreateCommand())
                {
                    ...
                    countQuery.Append("SELECT COUNT(DISTINCT c.\"Id\") FROM \"Comment\" c ");
                    countQuery.Append("LEFT JOIN \"User\" u ON c.\"UserId\" = u.\"Id\" ");
                    countQuery.Append("WHERE c.\"IsActive\" = true");

                    if (!string.IsNullOrEmpty(filtering.Username))
                    {
                        countQuery.Append(" AND LOWER(u.\"Username\") = LOWER(@Username)");
                        countCmd.Parameters.AddWithValue("@Username", filtering.Username.ToLower());
                    }
                    if (filtering.DateCreated != null)
                    {
                        countQuery.Append(" AND c.\"DateCreated\" >= @DateCreated");
                        countCmd.Parameters.AddWithValue("@DateCreated", filtering.DateCreated.Value);
                    }
                    if (filtering.IsReported.HasValue)
                    {
                        countQuery.Append(" AND c.\"IsReported\" = @IsReported");
                        countCmd.Parameters.AddWithValue("@IsReported", filtering.IsReported.Value);
                    }

                    countCmd.CommandText = countQuery.ToString();
                    totalComments = Convert.ToInt32(await countCmd.ExecuteScalarAsync());
                }

                using (var cmd = con.CreateCommand())
                {
                    StringBuilder query = new StringBuilder();
                    query.Append("SELECT c.\"Id\", c.\"CommentText\", c.\"IsActive\", c.\"UserId\" AS CommentUserId, c.\"EventId\" AS CommentEventId, c.\"TouristSitesId\" AS CommentTouristSitesId, c.\"StoryId\" AS CommentStoryId, c.\"DateCreated\", c.\"DateUpdated\", c.\"CreatedBy\", c.\"ParentComment\", c.\"NumberOfLikes\", c.\"IsReported\", ");
                    ...
                    query.Append("WHERE c.\"Id\" = ANY(@CommentIds)");

                    cmd.Parameters.AddWithValue("@CommentIds", commentIds.ToArray());
                    ...
                    using (var reader = await cmd.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            Guid currentCommentId = reader.GetGuid(reader.GetOrdinal("Id"));

                            if (!comments.TryGetValue(currentCommentId, out currentComment))
                            {
                                currentComment = MapComment(reader);
                                comments.Add(currentCommentId, currentComment);
                            }
            ...
            PagingInfo<ICommentModel> pagingInfo = new PagingInfo<ICommentModel>
            {
                List = comments.Values.ToList(),
                RRP = paging.RRP,
                PageNumber = paging.PageNumber,
                TotalSize = totalComments
            };
            return pagingInfo;
        }
        ...
        public async Task<List<ICommentModel>> GetAllParentCommentsAsync(Guid? eventId = null, Guid? touristSiteId = null, Guid? storyId = null)
        {
            ...
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    query.Append("AND c.\"ParentComment\" IS NULL ");

                    if(eventId != null)
                    {
                        cmd.Parameters.AddWithValue("@eventId", eventId);
                        query.Append("AND c.\"EventId\" = @eventId");
                    }
                    else if (touristSiteId != null)
                    {
                        cmd.Parameters.AddWithValue("@touristSiteId", touristSiteId);
                        query.Append("AND c.\"TouristSitesId\" = @touristSiteId");
                    }
                    else if (storyId != null)
                    {
                        cmd.Parameters.AddWithValue("@storyId", storyId);
                        query.Append("AND c.\"StoryId\" = @storyId");
                    }
        ...
        public async Task<List<ICommentModel>> GetAllChildCommentsAsync(Guid parentCommentId)
        {
            ...
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    query.Append("LEFT JOIN \"User\" u ON c.\"UserId\" = u.\"Id\" ");
                    query.Append("WHERE c.\"IsActive\" = true ");
                    query.Append("AND c.\"ParentComment\" = @parentComment");
        ...
        public async Task<List<ICommentModel>> GetCommentsByUserIdAsync(Guid userId)
        {
            Dictionary<Guid, ICommentModel> comments = new Dictionary<Guid, ICommentModel>();
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    query.Append("AND c.\"StoryId\" IS NOT NULL");
        ...
        public async Task AddCommentAsync(ICommentModel comment)
        {
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    StringBuilder query = new StringBuilder();
                    query.Append("INSERT INTO \"Comment\" (\"Id\", \"CommentText\", \"IsActive\", \"UserId\", \"EventId\", \"TouristSitesId\", \"StoryId\", \"DateCreated\", \"DateUpdated\", \"CreatedBy\", \"UpdatedBy\", \"ParentComment\", \"NumberOfLikes\", \"IsReported\") ");
                    query.Append("VALUES (@id, @text, @isActive, @userId, @eventId, @touristSiteId, @storyId, @dateCreated, @dateUpdated, @createdBy, @updatedBy, @parentComment, @numberOfLikes, @isReported)");
                    ...
                    cmd.Parameters.AddWithValue("@parentComment", comment.ParentComment ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@numberOfLikes", comment.NumberOfLikes ?? (object)DBNull.Value);
                    cmd.Parameters.AddWithValue("@isReported", comment.IsReported ?? (object)DBNull.Value);
            ...
            using (var con = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    if (comment.NumberOfLikes != null)
                    {
                        ...
                        {
                            cmd.Parameters.AddWithValue("@numberOfLikes", comment.NumberOfLikes);
                        }
                        query.Append("\"NumberOfLikes\" = @numberOfLikes, ");
                    }
                    if (comment.IsReported != null)
                    {
                        query.Append("\"IsReported\" = @isReported, ");
                        cmd.Parameters.AddWithValue("@isReported", comment.IsReported);
                    }
