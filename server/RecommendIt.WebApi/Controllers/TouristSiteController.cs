...
namespace GeoTagMap.WebApi.Controllers
{
    ...
    public class TouristSiteController : ApiController
    {
            ...
            )
        {
            ...
            TouristSiteFiltering filtering = new TouristSiteFiltering(name, popularity, rating, searchKeyword, country, city);
        ...
        [Authorize(Roles = "Admin")]
        [Route("mostliked")]
        public async Task<HttpResponseMessage> GetMostLikedTouristSitesAsync()
        {
            List<TouristSiteView> touristSiteViews = new List<TouristSiteView>();
            try
            {
                var touristSites = await _touristSiteService.GetMostLikedTouristSitesAsync();
                if (touristSites.Count() == 0)
                {
                    return Request.CreateResponse(HttpStatusCode.NoContent, "List is empty");
                }
                foreach (var touristSite in touristSites)
                {
                    touristSiteViews.Add(MapTouristSiteView(touristSite));
                }

                return Request.CreateResponse(HttpStatusCode.OK, touristSiteViews);
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.InternalServerError, ex.Message);
        ...
        private ITouristSitesModel MapTouristSite(TouristSiteRest touristSiteRest)
        {
            return new TouristSitesModel
            {
                ...
                NumberOfLikes = touristSiteRest.NumberOfLikes,
        ...
        private TouristSiteView MapTouristSiteView(ITouristSitesModel touristSite)
        {
            return new TouristSiteView
            {
                ...
                NumberOfLikes = touristSite.NumberOfLikes,

                Location = MapLocationView(touristSite.Location),
                GeoLocations = MapGeoLocationViews(touristSite.GeoLocations),
